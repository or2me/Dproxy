...
shadowsocks_config() {
	...
	while :; do
		echo -e "是否配置 ${yellow}Shadowsocks${none} [${magenta}Y/N$none]"y
...
backup_config() {
	sed -i "18s/=1/=$v2ray_transport/; 21s/=2333/=55555/; 24s/=$old_id/=$uuid/" $backup
	if [[ $v2ray_transport -ge 18 ]]; then
		sed -i "30s/=10000/=$v2ray_dynamic_port_start_input/; 33s/=20000/=$v2ray_dynamic_port_end_input/" $backup
	fi
	if [[ $shadowsocks ]]; then
		sed -i "42s/=/=true/; 45s/=6666/=44444/; 48s/=233blog.com/=littleYellow/; 51s/=chacha20-ietf/=$ssciphers/" $backup
	fi
